...
Message.prototype.toSwift = function () {
    ...
    if (typeof args === 'string') {
        ...
        pieces = _.map(args, function (v, arg) {
            ...
            if (firstLabel || isInitializing) {
                ...
                if (receiver[0].toUpperCase() === receiver[0]) {
                    isInitializing = true;
                    arg = _.lowerFirst(parts[1]);
                } else {
                    firstLabel = parts[0];
                    arg = 'with' + parts[1];
                }
    ...
    return `${receiver}${firstLabel ? '.' + firstLabel : ''}(${pieces.join(', ')})`;
};
...
NewObject.prototype.toSwift = function () {
    let args = this.args;
    return Message(this.cls, args).toSwift();
};
...
TypeCoercion.prototype.toSwift = function () {
    return `${this.val.toSwift()} as! ${this.coercedType}`;
};
