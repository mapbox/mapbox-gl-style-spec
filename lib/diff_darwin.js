...
let operations = {
    ...
    setPaintProperty: function (layerId, prop, value, cls, os) {
        ...
        return [varDecl(layerClass, layerVar, typeCast(layerClass, getLayerMsg)),
                varAssignment(`${layerVar}.${params[0]}`, params[1])];
    },
    ...
    setFilter: function (layerId, filter) {
        ...
        return [varDecl('MGLVectorStyleLayer', layerVar, typeCast('MGLVectorStyleLayer', getLayerMsg)),
                varAssignment(`${layerVar}.predicate`, predicateMsg)];
    },
    addSource: function (sourceId, source) {
        ...
        if ('source-layer' in source) {
            statements.push(varAssignment(`${sourceVar}.sourceLayerIdentifier`,
    ...
    setLayerZoomRange: function (layerId, minZoom, maxZoom) {
        ...
        return [varDecl('MGLStyleLayer', layerVar, getLayerMsg),
                varAssignment(`${layerVar}.minimumZoomLevel`, minZoom || 0),
                varAssignment(`${layerVar}.maximumZoomLevel`, maxZoom || 22)];
    },
    setCenter: function (center) {
        let makeCoordinateCall = fnCall('CLLocationCoordinate2DMake', [center[1], center[0]]);
        return [varAssignment('mapView.centerCoordinate', makeCoordinateCall)];
    },
    setZoom: zoom => [varAssignment('mapView.zoomLevel', zoom)],
    setBearing: bearing => [varAssignment('mapView.direction', bearing)],
    setPitch: function (pitch) {
        return [varDecl('MGLMapCamera', 'camera', 'mapView.camera'),
                varAssignment('camera.pitch', pitch),
                varAssignment('mapView.camera', camera)];
    },
};
