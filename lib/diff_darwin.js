...
let operations = {
    ...
    addSource: function (sourceId, source) {
        ...
        if ('url' in source) {
            ...
            let templates = _.map(source.tiles, str => valueToLiteral(str));
            ...
            let options = _.filter(_.map(sourceOptionKeys, function (js, objC) {
                if (source[js]) {
                    return `${objC}: ${valueToLiteral(source[js], true)}`;
                }
            }));
            if (options.length) {
