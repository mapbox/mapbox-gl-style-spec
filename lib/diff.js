...
var operations = {
    ...
    /*
     * { command: 'setLightProperty', args: [property, value] }
     */
...
function diffLight(before, after, commands) {
    before = before || {};
    after = after || {};

    var lightProps = ['anchor', 'direction', 'color', 'intensity'];

    for (var i in lightProps) {
        var prop = lightProps[i];

        if (!isEqual(before[prop], after[prop])) {
            commands.push({ command: operations.setLightProperty, args: [prop, after[prop]] });
        }
    }
}
...
function diffStyles(before, after) {
    ...
    try {
        ...
        diffLight(before.light, after.light, commands);
    } catch (e) {
