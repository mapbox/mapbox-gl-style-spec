...
function diffLayers(before, after, commands) {
    ...
    // An index of layers to be removed.
    var isRemovedIndex = {};
    var commandStartIndex = commands.length;

    // remove layers
    for (i = 0, d = 0; i < beforeOrder.length; i++) {
        layerId = beforeOrder[i];
        if (!afterIndex.hasOwnProperty(layerId)) {
            isRemovedIndex[layerId] = true;
    ...
    for (i = commandStartIndex; i < commands.length; i++) {
        // if this layers ref is being removed, this layer will be removed with
        // it without us having to do so.
        layerId = commands[i].args[0];
        if (isRemovedIndex[beforeIndex[layerId].ref]) {
            commands.splice(i, 1);
            i = i - 1;
        }
    }
