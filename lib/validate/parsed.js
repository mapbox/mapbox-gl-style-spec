...
module.exports = function(style, reference) {
    ...
    validate.color = function(key, val) {
        var ops = ['darken', 'lighten', 'desaturate', 'saturate', 'spin', 'fadeout', 'fadein', 'mix'];
        ...
        if (type !== 'string' && type !== 'array') {
            ...
            if (ops.indexOf(unbundle(val[0])) === -1) {
                error(key, val, 'expected one of [%s], %s found', ops.join(', '), val[0]);
            }
